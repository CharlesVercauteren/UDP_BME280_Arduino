...
Adafruit_BME280 bme;                    // Gebruikt I2C
...
void setup() {
  ...
  // Controleer of BME280 is aangesloten
    if (!bme.begin(0x76)) {
    Serial.println("Geen sensor gevonden, controleer uw opstellingen en herstart.");
    while (1);
  }
  ...
  if (packetSize) {
    ...
    switch (command) {
      case GET_TEMPERATURE:
          replyString = String(command) + " " + String(bme.readTemperature());
          break;
      case GET_HUMIDITY:
          replyString = String(command) + " " + String(bme.readHumidity());
          break;      
      case GET_PRESSURE:
          replyString = String(command) + " " + String(bme.readPressure()/100);
          break;
      default:
        replyString = String(command) + " " +"Unknown command.";
    }

    // Voeg newline toe voor command line gebruik
    //replyString += "\n";
